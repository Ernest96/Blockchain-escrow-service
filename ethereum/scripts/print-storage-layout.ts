...
for (const item of layout.storage) {
  ...
  console.log(
    `| ${item.slot.padStart(4)} | ${String(item.offset).padStart(6)} | ${item.label.padEnd(8)} | ${typeName.padEnd(39)} |`,
  );
}
...
if (dealType) {
  ...
  for (const member of info.members ?? []) {
    ...
    console.log(
      `| ${String(member.slot).padStart(4)} | ${String(member.offset).padStart(6)} | ${member.label.padEnd(8)} | ${typeName.padEnd(39)} |`,
    );
  }
}
